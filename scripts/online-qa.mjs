...
try {
  ...
  const third=await browser.newPage();await third.goto(url+'?room='+code);await third.getByRole('button',{name:/Vào phòng →/}).click();await third.getByText(/ROOM_FULL/).waitFor({timeout:25000});console.log('ROOM_FULL_OK');
  await third.close();
  await host.locator('canvas').focus();await host.keyboard.press('b');
  ...
  await guest.close();await host.getByRole('button',{name:'Kết nối lại',exact:true}).waitFor({timeout:75000});
  ...
  await expect.poll(async()=>Number(await guest.getByTestId('game-state').getAttribute('data-boss-hp'))).toBeLessThan(900);console.log('RECONNECT_RESTORES_BATTLE_OK');
