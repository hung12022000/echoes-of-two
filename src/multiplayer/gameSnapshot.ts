...
export function applySnapshot(state: Encounter, data: unknown, smooth = false): boolean {
  ...
  const merge = (current: Actor, incoming: Actor): Actor => {
    if (!smooth || Math.hypot(incoming.x - current.x, incoming.z - current.z) > 8) return { ...incoming };
    const blend = .46; const yawDelta = Math.atan2(Math.sin(incoming.yaw - current.yaw), Math.cos(incoming.yaw - current.yaw));
    return { ...incoming, x: current.x + (incoming.x - current.x) * blend, y: current.y + (incoming.y - current.y) * .6, z: current.z + (incoming.z - current.z) * blend, yaw: current.yaw + yawDelta * blend };
  };
  state.actors.hung = merge(state.actors.hung, data.actors.hung); state.actors.mei = merge(state.actors.mei, data.actors.mei);
